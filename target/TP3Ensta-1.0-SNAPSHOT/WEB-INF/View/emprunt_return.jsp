	            ...
	            <select id="id" name="id" class="browser-default">
                  	...
                  	<c:forEach items="${listEmpruntCurrent}" var="emprunt">
                  		<c:if test="${id!=emprunt.getId()}">
	              			<option value="${emprunt.getId()}" >"${livreInstance.getById(emprunt.getIdLivre()).getTitre()}", emprunté par ${membreInstance.getById(emprunt.getIdMembre()).getPrenom()}  ${membreInstance.getById(emprunt.getIdMembre()).getNom()}</option>
	              		</c:if>
                  		
                    	<c:if test="${id==emprunt.getId()}">
	              			<option value="${id}" selected >"${titreLivreSelected}", 
	              			emprunté par ${prenomMembreSelected}  
